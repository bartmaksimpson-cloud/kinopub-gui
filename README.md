...
- ⏯️ **Управление на лету** — пауза, отмена и повтор по каждому эпизоду отдельно; очередь сохраняется на диск и переживает перезапуск — докачка продолжается с места остановки.
...
Значения по умолчанию для новых загрузок (папка вывода, качество, контейнер, прокси), а также дополнительные папки для сканирования в Библиотеке, вход в kino.watch, установщик ffmpeg и обновление ПО. Хранятся в `~/.config/kinopub/gui.json`.
